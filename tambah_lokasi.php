...
<body>
  ...
  <main id="main">
      ...
      <div class="container mt-7">
        <div class="row gy-4">
          ...
          <div id="map" style="width: 100%; height: 400px;"></div>
            <script>
              ...
              var map = L.map('map').setView([-7.008099, 113.860220], 16);
        ...
        <form method="POST" action="" enctype="multipart/form-data">
          <div class="row">
            ...
            <div class="form-group mt-3">
              <label for="">Latlang</label>
              <textarea name="latlang" id="" cols="10" rows="10" class="form-control"></textarea>
            </div>
            <div class="form-group mt-3">
    ...
    if(isset($_POST['submit'])){
        ...
        $sql = mysqli_query($koneksi, "SELECT * FROM map WHERE id_map='$id_map'") or die(mysqli_error($koneksi));

        if(mysqli_num_rows($sql) == 0){
            $sql = mysqli_query($koneksi, "INSERT INTO map(id_map, id_perum, latlang, geojson) VALUES('', '$id_perum', '$latlang','$geojson')") or die(mysqli_error($koneksi));
            if($sql){
                echo '<script>alert("Berhasil menambahkan data map."); document.location="tambah_lokasi.php";</script>';
            }else{
...
<a href="assets/img/portfolio/<?php echo"$tampil[gambar]"?>" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="App 1"><i class="bx bx-plus"></i></a>
