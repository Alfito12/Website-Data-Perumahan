...
<body>
  ...
  <main id="main">
    ...
    if(isset($_POST['submit'])){
        ...
        $sql = mysqli_query($koneksi, "SELECT * FROM jalan WHERE id_jalan='$id_jalan'") or die(mysqli_error($koneksi));

        if(mysqli_num_rows($sql) == 0){
            $sql = mysqli_query($koneksi, "INSERT INTO jalan(id_jalan, id_perum, geojson) VALUES('', '$id_perum', '$geojson')") or die(mysqli_error($koneksi));
            if($sql){
                echo '<script>alert("Berhasil menambahkan data jalan."); document.location="tambah_jalan.php";</script>';
            }else{
                echo '<div class="alert alert-warning">Gagal melakukan proses tambah data jalan.</div>';
            }
        }else{
            echo '<div class="alert alert-warning">Gagal, Jalan sudah terdaftar.</div>';
        }
    }
    ?>
